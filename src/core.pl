...
apply_forall(G1, A1, G2, A2) :-
	forall(apply(G1, A1), apply(G2, A2)).

apply_catch(C1, A1, C2) :-
	C1=closure(P, T1),
	C2=closure(Q, T2),
...
list_of(X, Closure, Args, L) :-
	Closure=closure(Pred, Env),
	bagof(X, apply(Pred, [Env|Args]), L).

list_of_atom(X, Pred, Args, L) :-
	bagof(X, apply(Pred, Args), L).
...
fopen(Filename, Mode, File) :-
	...
	;   Mode="update"
	->  open(Filename, update, File)
	;   throw("not a correct mode for 'open'.")
	).
...
obj_call(Env, Obj, Method, Arg) :-
	get(Env, Obj, _t1),
	get(_t1, Method, _t2),
	apply(_t2, Arg).

env_call(Env, Method, Arg) :-
	get(Env, Method, _t2),
	apply(_t2, Arg).

obj_get(Env, Obj, Method, X) :-
	get(Env, Obj, _t1),
	get(_t1, Method, X).

env_get(Env, Method, X) :-
	get(Env, Method, X).

closure_call(Env, Closure, Arg) :-
	closure(Method, MethodEnv)=Closure,
	_t2=Method,
	Arg2=[MethodEnv|Arg],
	apply(_t2, Arg2).
